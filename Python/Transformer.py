...
# Modified based on https://github.com/ctxj/Time-Series-Transformer-Pytorch/blob/main/transformer_model.ipynb

input_window = 10  # number of input steps
output_window = 1 # open loop prediction
batch_size = 4
epochs = 200 
lr = 0.001
device = torch.device("cuda" if torch.cuda.is_available() else "cpu") 

df = pd.read_csv('Model.csv') # flow rate (Jan 2021 - Feb 2021)
close = np.array(df['Rate'])
print(close)


logreturn=close
csum_logreturn=close

fig, axs = plt.subplots(2, 1)
axs[0].plot(close, color='red')
axs[0].set_title('Flow rate')
